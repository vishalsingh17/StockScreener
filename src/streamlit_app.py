...
from utils.OtaUpdater import OTAUpdater
from utils.Changelog import VERSION
...
def on_start_button_click():
    ...
    while int(os.environ.get('SCREENIPY_SCREEN_COUNTER')) < 100:
      ...
      if os.environ.get('SCREENIPY_REQ_ERROR') and "TRUE" in os.environ.get('SCREENIPY_REQ_ERROR'):
        ...
        ac.error(':disappointed: Failed to reach Stock Screener server!')
...
ac.markdown("""
    <style>
    .main-title {
        margin-top: 30px;  /* Adjust this value to control the title's vertical position */
        text-align: center; /* Optional: To center the title */
    }
    .subheader-title {
        margin-top: 10px;  /* Adjust this for the subheader as needed */
...
ac.markdown('<h1 class="main-title">📈 Stock Screener</h1>', unsafe_allow_html=True)
...
with tab_similar:
  ...
  if similar_search_button:
    ...
    if result:
      ...
      st.error('No similar stocks found!', icon='🚫')
with tab_about:
  st.success(f'Stock Screener v{VERSION}', icon='🔍')
  ac, bc = st.columns([2,1])
  ac.info("""
👨🏻‍💻 Developed and Maintained by: Vishal Singh
          
🏠 Home Page: https://github.com/vishalsingh17/StockScreener
          
⚠️ Read/Post Issues here: https://github.com/vishalsingh17/StockScreener/issues
          
📣 Join Community Discussions: https://github.com/vishalsingh17/StockScreener/discussions
          
⬇️ Download latest software from https://github.com/vishalsingh17/StockScreener/releases/latest
